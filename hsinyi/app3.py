        ...
        with gr.TabItem("👤 信用貸款審核專區"):
            ...
            with gr.Row():
                with gr.Column(elem_classes="option-style"):
                    ...
                    p_edu = gr.Dropdown(label="教育程度", choices=["高中/職", "副學士", "學士", "碩士", "博士"],
                                        value="學士")
            ...
            # 事件處理 (精度驗證)
            l_rate.change(fn=validate_precision, inputs=[l_rate], outputs=[l_rate])
            l_percent.change(fn=validate_precision, inputs=[l_percent], outputs=[l_percent])
